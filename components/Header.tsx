...
const Header = () => {
  ...
  return (
    <>
      <nav
        className={`p-4 flex justify-between px-10 poppins antialiased transition-all duration-1000 ease-in-out z-50 fixed shrink-0 overflow-hidden ${
          scrolled
            ? " top-4 rounded-lg  left-10 right-10 shadow-sm backdrop-blur-xl bg-[#B0D4E3]/50 "
            : "relative top-0 w-full bg-[#B0D4E3]"
        }`}
      >
        <div className="flex w-[30%] items-center justify-between 0">
          <div className="h-[2.31em] w-12 object-cover rounded-full">
            <Image
              src={logo}
              alt="Bible and Book ministries logo"
          ...
          <ul className="flex justify-between w-[80%] text-sm overflow-hidden">
            {navItems.map(({ label, href }, idx) => (
              <Link key={idx} href={href}>
                <li
                  className={`cursor-pointer hover:scale-[1.1] transition duration-300 ${
                    pathName === href ? "text-blue-400" : "text-black"
                  }`}
                >
                  {label}
                </li>
        ...
        <div className="w-[40%] relative">
          <div className="absolute top-[50%] left-4 -translate-y-[55%] text-xl">
            {" "}
            <CiSearch />
          </div>
          <input
            type="text"
            name="search-bar"
            className="w-full h-full border border-black p-4 px-4 pl-10 outline-0 rounded-full shadow-lg focus:shadow-2xl duration-150 transition antialiased"
